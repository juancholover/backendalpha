...
public class Silabo extends AuditableEntity {
    ...
    /**
     * INMUTABILIDAD: Indica si este sílabo ya fue asignado a alguna oferta de curso.
     * Una vez que en_uso = true, NO SE PUEDE EDITAR ni ELIMINAR.
     * Solo se puede crear una nueva versión.
     */
    @Column(name = "en_uso", nullable = false)
    private Boolean enUso = false;

    /**
     * Almacenamiento del archivo PDF del sílabo.
     * Decisión: Guardar la RUTA (URL) en lugar del BLOB por:
     * 1. Escalabilidad (10 años = miles de PDFs, PostgreSQL lento con BLOBs grandes)
     * 2. Backups más rápidos (base de datos más liviana)
     * 3. CDN-friendly (AWS S3, Azure Blob, MinIO localmente)
     * Formato: "s3://bucket-silabos/2025/programacion-i-v2.pdf"
     */
    @Column(name = "url_pdf_archivo", length = 1000)
    private String urlPdfArchivo;
     ...
     * REGLA DE INMUTABILIDAD: No se puede modificar si ya está en uso
     */
    public boolean esModificable() {
        if (this.enUso != null && this.enUso) {
            return false; // INMUTABLE si ya fue usado en una oferta
        }
        return "BORRADOR".equals(this.estado) || "EN_REVISION".equals(this.estado);
    }

    /**
     * Marca el sílabo como "en uso" cuando se asigna a una CursoOfertado.
    ...
    /**
     * Verifica si el sílabo está bloqueado para edición
     */
    public boolean estaInmutable() {
        return this.enUso != null && this.enUso;
    }
    ...
    public void enviarARevision() {
        if (estaInmutable()) {
            throw new IllegalStateException("No se puede modificar un sílabo que ya está en uso. Crea una nueva versión.");
        }
